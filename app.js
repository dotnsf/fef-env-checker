...
app.get( '/api/jsons/:source_id', async function( req, res ){
  res.contentType( 'application/json; charaset=utf-8' );

  var _checked = ( 'checked' in req.query ? req.query.checked : null );
  var checked = ( _checked ? parseInt( _checked ) : -1 );
  var source_id = req.params.source_id;
  var jsons = await app.readJsonsBySourceId( source_id, checked );
  res.write( JSON.stringify( { status: true, source_id: source_id, jsons: jsons }, null, 2 ) );
  res.end();
});

app.post( '/api/json/:json_id/:checked', async function( req, res ){
  res.contentType( 'application/json; charaset=utf-8' );

  var json_id = req.params.json_id;
  var checked = parseInt( req.params.checked );
  var b = await app.setJsonChecked( json_id, checked );
  res.write( JSON.stringify( { status: true, json_id: json_id, checked: checked, b: b }, null, 2 ) );
  res.end();
});
...
app.delete( '/api/url/:url_id', async function( req, res ){
  ...
  var url_id = req.params.url_id;
  var r = await app.removeDataByUrlId( url_id );
...
app.readSourceBodyById = async function( source_id ){
  return new Promise( async function( resolve, reject ){
    var conn = null;
    var body = '';
    try{
      conn = await pg.connect();
      if( conn ){
        var sql = "select body from sources where id = $1";
        var query = { text: sql, values: [ source_id ] };
        conn.query( query, function( err, result ){
          if( err ){
            console.log( err );
          }else{
            if( result.rows.length > 0 && result.rows[0].source ){
...
app.readJsonsBySourceId = async function( source_id, checked ){
  return new Promise( async function( resolve, reject ){
    var conn = null;
    var jsons = [];
    try{
      conn = await pg.connect();
      if( conn ){
        var sql = "select id, start_index, end_index, body, checked, created from jsons where source_id = $1 order by created";
        var query = { text: sql, values: [ source_id ] };
...
app.setJsonChecked = async function( json_id, checked ){
  return new Promise( async function( resolve, reject ){
    var b = false;
    var conn = null;
    try{
      conn = await pg.connect();
      if( conn ){
        var sql = "update jsons set checked = $1 where id = $2";
        var query = { text: sql, values: [ checked, json_id ] };
        conn.query( query, function( err, result ){
          if( err ){
            console.log( err );
          }else{
...
app.storeSource = async function( url_id, source, body ){
  return new Promise( async function( resolve, reject ){
    var conn = null;
    try{
      conn = await pg.connect();
...
app.storeJson = async function( source_id, start_index, end_index, body ){
  return new Promise( async function( resolve, reject ){
    var conn = null;
    try{
      conn = await pg.connect();
...
app.deleteJsonsBySourceId = async function( source_id ){
  return new Promise( async function( resolve, reject ){
    var conn = null;
    try{
      conn = await pg.connect();
                  ...
                  $('script').each( async function(){
                    ...
                    if( src && src.toLowerCase().endsWith( '.js' ) ){
                      ...
                      if( r1 && r1.body ){
                        try{
                          var body1 = r1.body;
                          var r2 = await app.storeSource( url_id, src1, body1 );
                          if( r2 && r2.status ){
                            //console.log( { src1 }, { r2 } );
                            var source_id = r2.id;
                            await app.deleteJsonsBySourceId( source_id );
            
                            var results = app.JSONdetect( body1 );
                            //console.log( {results} );
                            for( var i = 0; i < results.length; i ++ ){
                              var r3 = await app.storeJson( source_id, results[i].start_index, results[i].end_index, results[i].text );
...
app.removeDataByUrlId = async function( url_id ){
  return new Promise( async function( resolve, reject ){
    var conn = null;
    try{
      conn = await pg.connect();
          ...
          if( err0 ){
            ...
            if( result0.rows.length > 0 && result0.rows[0].id ){
              ...
              if( source_ids.length > 0 ){
                var sql1 = "delete from jsons where source_id = any($1::varchar[])";
